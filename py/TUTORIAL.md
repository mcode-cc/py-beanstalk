...
(already running) beanstalkd server:
    ...
    >>> import beanstalkm
    >>> beanstalk = beanstalkm.Connection(host='localhost', port=14711)
...
Note that beanstalkm requires job bodies to be strings, conversion to/from
...
If, for whatever reason, you cannot use PyYAML, you can still use beanstalkm
and just leave the YAML responses unparsed. To do that, pass `parse_yaml=False`
when creating the `Connection`:

    >>> beanstalk = beanstalkm.Connection(host='localhost',
    ...                                   port=14711,
    ...                                   parse_yaml=False)
